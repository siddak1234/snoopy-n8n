...
COMPOSE_ARGS="--env-file .env.local -f docker-compose.yml -f docker-compose.local.yml"
...
docker compose $COMPOSE_ARGS exec n8n sh -lc 'echo "$GOOGLE_APPLICATION_CREDENTIALS"'

echo "[5/5] Verifying pdf-lib allowlist and runtime availability"
rg -n "NODE_FUNCTION_ALLOW_EXTERNAL=pdf-lib" .env.local.example
docker compose $COMPOSE_ARGS exec n8n sh -lc 'node -e "require(\"pdf-lib\"); console.log(\"pdf-lib ok\")"'
